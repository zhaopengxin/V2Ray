...
getCert() {
    ...
    if [[ -z ${CERT_FILE+x} ]]; then
        ...
        # nginx watchdog: 每5分钟检测，没运行就自动启动
        (crontab -l 2>/dev/null | grep -v nginx-watchdog; echo '*/5 * * * * systemctl is-active --quiet nginx || systemctl start nginx # nginx-watchdog') | crontab -
